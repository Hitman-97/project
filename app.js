...
app.get('/items/:pid', async (req, res) => {
  try {
    const item = await Item.findById(req.params.pid).exec();
    if (!item) return res.status(404).json({ message: 'Item not found' });
    res.json(item);
  } catch (e) {
    res.status(400).json({ message: 'Invalid item ID', error: e.message });
  }
});
...
app.delete('/checkout/:itemId', authenticateToken, async (req, res) => {
  try {
    const clientId = req.clientId;
    const itemId = req.params.itemId;
    ...
    res.status(500).json({ message: 'Error removing item from checkout', error: e.message });
  }
});
